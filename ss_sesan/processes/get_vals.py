...
def calcDataCoverage(db, device_id_3, mun_id, login):
    ...
    if int(if_cu) != 0:
        ...
        if int(if_cu2) != 0:
            query = "SELECT (100/count(*)) * (select count(sem_comunidad_totales) from %s.maintable_msel_sem_comunidad_totales)  FROM sesan_v2.centros_urbanos where munic_id=%s;" % (mun_id)
